...
public class TestUiCode {
    ...
    TradeSettings settings1 = new TradeSettings();
        ...
        order1.setOrderQty(20L);
        printOrder(order1);
        printOrder(order1.getReversalOrder());
        order1.setTradeType(order1.getTradeType().getOppositeDirection());
        printOrder(order1);
        printOrder(order1.getStoplossOrder());
 ...
 static void printOrder(Order order){
        String orderString = order.getScrip()+"\t"+order.getOrderQty()+"\t"+order.getTradeType();
